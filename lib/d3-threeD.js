...
function transformSVGPath(pathStr) {
  ...
  while (idx <= len) {
    ...
    switch (activeCmd) {
      ...
      case 'z':
        canRepeat = false;
        var point = path.curves[0].getPoint(0);
      ...
      case 'A':
        ...
        if (sf && deltaAng < 0)
          deltaAng += Math.PI * 2;
console.log("center", cx, cy, "radius", rx,"startAng", startAng, "endAng", startAng + deltaAng);
        // because the getPoints implemenation for arcs assumes things are
        //  relative to our last point in a weird fashion (seems to assume it's
        //  not dealing with another arc? file a bug if so!), let's use moveTo
        //  0, 0 for now to avoid that.
        path.moveTo(0, 0);
...
function commonSetAttribute(name, value) {
  switch (name) {
    ...
    case 'd':
      ...
      this.scale.set(3 * UNIT_SIZE, 3 * UNIT_SIZE, 3 * UNIT_SIZE);
...
function fabVis() {
  ...
  */
  ...
  spotlight.position.set(0, 0, 100000);
  scene.add(spotlight);
